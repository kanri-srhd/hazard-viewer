...
# INTERNAL LOGIC：FACT CONSISTENCY ENFORCEMENT（AI内部ロジック）

このセクションは、AIアシスタント（ChatGPT）が  
**SRHD Hazard Viewer プロジェクト内で“誤前提・事実無視・勝手な推論”を防ぐために  
必ず内部で実行すべき自己監督ロジック**である。

ユーザー（人間）が理解する必要はないが、  
AI は以下を回答生成前に自動的に実行する。

## Rule 1 — 事実照合（Answer Attempt Before Rule）
AI は、ディレクトリ操作・ファイル言及・パス提案・移動・削除・追加など  
**構造に直接関わる回答を行う前に、最新のディレクトリ構造と照合する。**

- 参照するフォルダ／ファイルが **存在しない** 場合  
  → 回答を停止し、ユーザーに「最新の構造提示」を要求する。

## Rule 2 — 暗黙のフォルダ生成禁止（Never Create New Folders Implicitly）
AI はユーザーが明示的に依頼しない限り、  
**存在しないフォルダを勝手に作る／提案することを禁止する。**

（例：tools/deprecated など、ユーザーが提示していないパスを前提にしてはならない）

## Rule 3 — 事実外パス禁止（Never Use Paths Not Present in State）
AI は回答中に使用するパスが  
**最新のプロジェクト構造に実在するものかを必ず確認**する。

一致しない場合は必ずユーザーに確認を求める。

## Rule 4 — 状態整合チェック（State Alignment Required）
新しい作業に入る前に AI は内部で次をチェックする：

- 対象フォルダは存在するか？  
- 対象ファイルは存在するか？  
- 現実（ユーザー提示の構造）と矛盾していないか？  

矛盾があれば回答を停止し、事実の再提示を依頼する。

## Rule 5 — 曖昧性ゼロ原則（If Ambiguous, Ask）
曖昧なパスや複数の可能性があるディレクトリが登場した場合、  
AI は **推論せず、想像せず、必ずユーザーに確認する。**

（例：「tools のどこに置きますか？」「archive を作成しますか？」など）

## Rule 6 — 絶対優先順位（User State > Guide > Memory > Assumption）
AI の判断優先度は以下で固定される：

1. **ユーザーが直近で提示したディレクトリ構造（絶対優先）**  
2. 本ガイド（AIアシスタントガイド）の規範  
3. 過去会話の情報  
4. ChatGPT の内部推測（最下位）

AI は、この優先順位を逆転してはならない。

---

本 INTERNAL LOGIC により、  
AI はディレクトリ構造やファイル位置に関する誤った推論を行わず、  
常に「存在する事実」を基準に動作する。
